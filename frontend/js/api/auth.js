/**
 * Authentication API Client (low-level HTTP wrapper)
 *
 * ============================================================
 * ROLE IN THE ARCHITECTURE
 * ============================================================
 * This file defines `AuthAPI` — a thin wrapper around fetch() that
 * knows how to call every `/api/auth/*` endpoint plus `/users/profile`
 * (for "who am I"). It does NOT manage token storage; that's the job
 * of `services/authService.js` (which exposes `window.authService`).
 *
 * New pages should use `window.authService` — not this class directly.
 * `AuthAPI` is kept as a low-level seam so the upper-level service can
 * be swapped or mocked without changing transport behaviour.
 *
 * For backwards compat, `window.AuthAPI` still exists. Pages that
 * instantiate `new AuthAPI()` directly continue to work — the storage
 * helper methods (getToken/setToken/etc.) are passed through to
 * window.authService so there's one source of truth even when called
 * via the legacy API.
 *
 * ============================================================
 ...
 *    TWICE in the class body (lines 81+177 and 102+196). JavaScript
 *    silently uses the last def, so the second copy (which returns
 *    actual errors) was overwriting the first (which suppressed errors
 *    to prevent email enumeration). The anti-enumeration intent was
 *    defeated. Now there's exactly one definition per method, and
 *    `forgotPassword` always returns success.
 *  - Missing endpoints added: `verifyEmail`, `resendVerification`,
 *    `updatePassword`, `getMe`. The backend (batch 1 authController)
 *    has these; the original API client didn't.
 *  - `getUser()` did `JSON.parse(localStorage.getItem(...))` with no
 *    try/catch. If localStorage was tampered with (or partially written
 *    during a crash), the app would crash on next load. Now wrapped.
 *  - `isAuthenticated()` only checked token existence. A user with an
 *    expired token would appear authenticated until the next API call
 *    failed. Now also parses the JWT and checks `exp`.
 *  - New: 401-refresh interceptor. When a token expires server-side,
 *    the request method auto-refreshes and retries once. Without this,
 *    the user gets logged out the first time they make an API call
 *    after their access token expires — even though their refresh
...
class AuthAPI {
    ...
    getUser() {
        // FIX: original threw if localStorage was tampered. Now safe.
        try {
    ...
    // ============================================================
    // FIX: `isAuthenticated()` used to just check token presence. We
    ...
    async _request(path, options = {}, isRetry = false) {
        ...
        // 401 handling: try to refresh, then retry the original request.
    ...
    // FIX: One definition only. Always treats the response as success
